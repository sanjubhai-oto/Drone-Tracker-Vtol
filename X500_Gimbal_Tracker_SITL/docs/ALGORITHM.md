# X500 Gimbal Tracker X500 gimbal Proximity Guidance Algorithm

This folder implements the full logic described in the supplied PDF: two X500 gimbal airframes (hunter + target), GPS-based pursuit, vision-based final confirmation, and a proximity-triggered event that stops the target motors, lets it freefall, and disarms on ground impact.

## Mission Phases

1. Spawn two X500 gimbal SITL vehicles.
...
- Start range: 40 m.
- Trigger radius: 25 m.
- Takeoff gate: both X500 gimbals must be above 20 m before any horizontal motion.
