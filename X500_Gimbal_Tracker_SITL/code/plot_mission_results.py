#!/usr/bin/env python3
"""Generate dependency-free SVG/HTML mission graphs from x500_gimbal_telemetry.json."""
...
def main() -> None:
    ...
    header = text(28, 42, "X500 Gimbal Tracker X500 gimbal SITL Result - Hunter follows target and triggers at 25m", 24, "#f8fafc")
    ...
    svg_path = OUT_DIR / "x500_gimbal_mission_graph.svg"
    html_path = OUT_DIR / "x500_gimbal_mission_graph.html"
    svg_path.write_text(svg)
    html_path.write_text(f"<!doctype html><meta charset='utf-8'><title>X500 Gimbal Tracker Mission Graph</title>{svg}")
    print(svg_path)
    print(html_path)
