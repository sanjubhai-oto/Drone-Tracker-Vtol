...
def main() -> None:
    ...
    if not px4_dir.exists():
        fail(f"PX4 directory not found: {px4_dir}. Set MOTHDRONE_PX4_DIR=/path/to/PX4-Autopilot")
    ...
    if not model_sdf.exists():
        fail(f"PX4 Gazebo standard_vtol model missing: {model_sdf}")
    ok(f"PX4 standard_vtol model: {model_sdf}")
    ...
    print("[CHECK] Environment looks ready for Mothdrone SITL.")
